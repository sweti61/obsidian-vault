...
Für die lokale Entwicklung wird bevorzugt verwendet:

```powershell
hugo server -D --cleanDestinationDir --disableFastRender --noHTTPCache --ignoreCache
```

Für den Produktions-Build wird bevorzugt verwendet:
...
Die Entscheidung basiert auf der tatsächlichen Nutzung: Der überwiegende Teil der Impressionen und Klicks kommt über Mobilgeräte.

### 5.1 Konsequenzen für CSS

Basisregeln werden für kleine Viewports geschrieben.

Größere Layouts werden anschließend mit `min-width`-Media-Queries ergänzt.
...
Desktop wird nicht als Ausgangszustand definiert und anschließend für Mobilgeräte zurückgebaut.

### 5.2 Konsequenzen für Tachyons

Die mobile Klasse bildet den Ausgangspunkt.
...
Responsive Erweiterungen folgen damit der gleichen Mobile-First-Logik.

### 5.3 Konsequenzen für Navigation

Das Burger-Menü ist die primäre Navigation für alle Viewports.

Damit gibt es:

- keine separate Desktop-Navigation
- keine separate Mobile-Navigation
- nur eine Navigationslogik
- nur einen Satz Menüpunkte
- nur eine JavaScript-Implementierung

Social-Media-Links können innerhalb des geöffneten Menüs zusätzlich mit Icons dargestellt werden.

### 5.4 Konsequenzen für Typografie

Schriftgrößen werden zunächst für kleine Displays definiert.

Größere Schriftgrößen werden nur dort ergänzt, wo sie auf breiteren Viewports sinnvoll sind.

Fließtext muss auf kleinen Bildschirmen ohne Zoom gut lesbar sein.

Zeilenlänge, Abstand und Zeilenhöhe werden zuerst mobil geprüft.

### 5.5 Konsequenzen für Abstände und Layout

Abstände werden zuerst für schmale Bildschirme festgelegt.

Größere horizontale Abstände werden erst für Tablet und Desktop ergänzt.

Mehrspaltige Komponenten beginnen grundsätzlich einspaltig.

### 5.6 Konsequenzen für Bilder

Das Bildsystem wird auf mobile Nutzung optimiert.

Der Browser soll bei kleinen Viewports keine unnötig großen Bilddateien laden.

Hugo Image Processing erzeugt deshalb mehrere sinnvolle Bildgrößen.

`srcset` und `sizes` sollen so definiert werden, dass kleine Geräte bevorzugt kleine Varianten erhalten.

PNG bleibt als Quelldatei möglich. Die Frontend-Ausgabe kann je nach Bildart optimiert werden.

### 5.7 Konsequenzen für Bedienung

Hover darf niemals Voraussetzung für Bedienung oder Informationszugang sein.

Interaktive Elemente benötigen ausreichend große Touch-Flächen.

Buttons, Menüpunkte und Icons müssen auch mit dem Finger gut bedienbar sein.

Fokuszustände bleiben zusätzlich für Tastaturbedienung sichtbar.

### 5.8 Konsequenzen für Performance

Performance wird primär aus Sicht mobiler Geräte betrachtet.

Besonders zu beachten sind:

- kleine Bilddateien
- wenig JavaScript
- keine unnötigen Webfonts oder Schriftschnitte
- keine eingebetteten YouTube-Player
- keine großen Frontend-Bibliotheken
- möglichst wenige blockierende Ressourcen

### 5.9 Konsequenzen für Tests

Jede neue Komponente wird zuerst auf einem schmalen Viewport geprüft.
...
Erst wenn die mobile Darstellung funktioniert, werden Erweiterungen für größere Viewports ergänzt.

---

## 6. Hugo-Umgebungen

Für `sweti.de` wird zwischen Entwicklungs- und Produktionsumgebung unterschieden.

Dabei gilt verbindlich:

> **Eine gemeinsame `config.toml`. Hugo-Environment über Hugos natives Environment-System. Templates prüfen `hugo.IsProduction` bzw. `hugo.IsDevelopment`; keine eigene Abhängigkeit von `$Env:HUGO_ENV`.**

Es werden keine zusätzlichen Environment-Konfigurationsdateien benötigt.

Die ältere Vorgehensweise, wie sie beispielsweise in älteren Ananke-Versionen vorkommen kann,

```go-html-template
{{ if eq (getenv "HUGO_ENV") "production" }}
```

wird für `sweti.de` nicht übernommen.

Stattdessen werden die nativen Hugo-Funktionen verwendet.
...
Die lokale Entwicklungsumgebung soll:

- kein Google Analytics laden
- keine produktionsspezifischen Tracking-Skripte laden
- nicht für Suchmaschinenindexierung gedacht sein
- Änderungen möglichst vollständig und ohne störende Cache-Effekte anzeigen
...
Die Produktionsumgebung darf:

- Google Analytics laden
- produktionsspezifische Meta-Tags ausgeben
- indexierbare `robots.txt`-Regeln verwenden
- minimierte Assets erzeugen

### 6.3 Konsequenz für Templates

Produktionsabhängige Logik wird ausschließlich über Hugo selbst gesteuert.
...
Damit hängt das Projekt nicht von einer extern gesetzten PowerShell-Umgebungsvariable ab.
...
## 15. Bildformat und Quelldateien
...
## 23. Reihenfolge der Implementierung
...
## 26. Verbindliche Architekturentscheidung
...
Zusätzlich gelten verbindlich:

> **Mobile First:** Die mobile Darstellung ist die primäre Design- und Implementierungsbasis von `sweti.de`. Desktop-Layouts sind Erweiterungen der mobilen Grundstruktur, nicht umgekehrt.

> **Umgebungen:** Eine gemeinsame `config.toml`. Hugo-Environment über Hugos natives Environment-System. Templates prüfen `hugo.IsProduction` bzw. `hugo.IsDevelopment`; keine eigene Abhängigkeit von `$Env:HUGO_ENV`.
